#!/usr/bin/env bash
# The full order-demo experiment matrix: 3 trace sizes x P0..P5.
...
OUT="demo/perf/results/order.jsonl"
...
grep "order_100k" demo/perf/traces/CHECKSUMS.sha256 | (cd demo/perf/traces && shasum -a 256 -c -) \
  || { echo "checksum mismatch on regenerated trace"; exit 1; }

for size in 1k 10k 100k; do
  for p in 0 1 2 3 4 5; do
    $PY -m demo.perf.run_experiment --demo order \
      --trace "demo/perf/traces/order_${size}.jsonl" \
      --policies "$p" --reps 5 --out "$OUT"
  done
done

echo
echo "== order: summary (median s / events per s / peak MB) =="
$PY - <<'PYEOF'
import json
rows = [json.loads(l) for l in open("demo/perf/results/order.jsonl")]
